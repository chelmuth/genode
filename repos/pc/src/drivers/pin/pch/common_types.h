...
struct Pin_driver::Index
{
	...
	static Index from_xml(Xml_node const &node)
	{
		...
		if (index > 31) {
			warning("'index' attribute too large ", node);
			throw Invalid();
		}
...
{
	enum Value { RISING = 0, FALLING = 1, HIGH = 2, LOW = 3, EDGES = 4, DISABLE = 5 } value;

	/* SMI and NMI fields are read-only according to spec */
	enum Route { NOROUTE = 0, IOXAPIC = 1, SCI= 2 } route;

	class Invalid : Exception { };

	static Irq_trigger from_xml(Xml_node const &node)
	{
		Route routing = NOROUTE;

		if (node.has_attribute("route")) {
			auto const value = node.attribute_value("route", String<10>());
			if (value == "ioxapic") routing = IOXAPIC;
			if (value == "sci")     routing = SCI;
		}

		if (!node.has_attribute("irq"))
			return Irq_trigger { DISABLE, routing };

		auto const value = node.attribute_value("irq", String<10>());

		if (value == "rising")  return Irq_trigger { RISING, routing };
		if (value == "falling") return Irq_trigger { FALLING, routing };
		if (value == "high")    return Irq_trigger { HIGH, routing };
		if (value == "low")     return Irq_trigger { LOW, routing };
		if (value == "edges")   return Irq_trigger { EDGES, routing };
		if (value == "disable") return Irq_trigger { DISABLE, routing };
...
struct Pin_driver::Attr
{
	...
	static Attr disabled()
	{
		return { { Pull::DISABLE },
		         { Function::DISABLE },
		         { Irq_trigger::DISABLE, Irq_trigger::NOROUTE },
		         false,
