...
int krishna_log = false;

struct G {
	char const    *ctx;
	unsigned long  ctx2;
	G(char const *ctx, unsigned long ctx2) : ctx(ctx), ctx2(ctx2) { krishna_log_consumed(ctx, ctx2); }
	~G() { krishna_log_consumed(ctx, ctx2+1); }
};

static bool _has_method(ACPI_HANDLE handle, char const *method)
{
	ACPI_HANDLE tmp;

	return ACPI_SUCCESS(AcpiGetHandle(handle, ACPI_STRING(method), &tmp));
}
...
class Battery : Acpica::Callback<Battery> {
	...
	private:
		...
		{
G volatile _g0(__func__, __LINE__);
krishna_log = true;
...
Genode::log("----- battery @ ", &_battery,
            " _BIF=", _has_method(_sb, "_BIF"),
            " _BIX=", _has_method(_sb, "_BIX"),
            " _BST=", _has_method(_sb, "_BST"));
		}
...
G volatile _g0(__func__, __LINE__);
			...
			ACPI_STATUS res = AcpiGetName (_sb, ACPI_SINGLE_NAME, &battery_name);
			if (ACPI_FAILURE(res))
