...
class Format_command
{
	...
	public:
		...
		explicit Format_command(const char *format)
		{
			...
			/* check for %$x syntax */
			prefix = (format[consumed] == '#') || (format[consumed] == '.') || (format[consumed] == '-');
			...
			/* skip precision modifier */
			if (format[consumed] == '.' && format[++consumed])
				...
				if (cmd.numeric()) {
					switch (cmd.length) {
						...
						case Format_command::SIZE_T:
							...
							numeric_arg = va_arg(list, unsigned long);
							break;
