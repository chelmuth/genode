...
class Vfs_server::Root : public Root_component<Session_component>
{
	private:
		...
		Create_result _create_session(const char *args, Node const &policy)
		{
			...
			if ((ram_used.value > ram_quota.value) || (cap_used.value > cap_quota.value)) {
				...
				if (cap_used.value > cap_quota.value)
		...
		{
			Ram_quota more_ram  = ram_quota_from_args(args);
			Cap_quota more_caps = cap_quota_from_args(args);
	...
	public:
		...
		:
			...
			_env(env), _vfs_env(vfs_env), _config(config),
			_io_progress_handler(io_progress_handler)
		{
			env.parent().announce(env.ep().manage(*this));
		}

		void handle_io_progress()
		{
			bool yield = false;
...
struct Vfs_server::Main : Entrypoint::Io_progress_handler
{
	Env &_env;
	...
	void _handle_config()
	{
		 ...
		 */
		_root.handle_io_progress();
	}

	/**
	 * Entrypoint::Io_progress_handler interface
	 */
	void handle_io_progress() override { _root.handle_io_progress(); }
...
{
	static Vfs_server::Main main { env };
}
